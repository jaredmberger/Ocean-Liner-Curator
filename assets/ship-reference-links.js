...
(function(){
  ...
  Promise.all([
  ...
  ]).then(([builders,operators])=>{
    const builderShip=(builders?.ships||[]).find(s=>String(s.path||'').replace(/\.html$/,'')===path||String(s.url||'').replace(/^https?:\/\/[^/]+/,'').replace(/\/$/,'')===path);
    const operatorShip=(operators?.ships||[]).find(s=>String(s.path||'').replace(/\.html$/,'')===path||String(s.url||'').replace(/^https?:\/\/[^/]+/,'').replace(/\/$/,'')===path);
    const links=[];
    ...
    if(!links.length&&operatorShip?.operator){
      const entity=(operators?.operators||[]).find(x=>x.name===operatorShip.operator);
      if(entity?.id)links.push(`<a href="/shipping-lines#line-${encodeURIComponent(entity.id)}">Line: ${esc(entity.name)}</a>`);
    }
